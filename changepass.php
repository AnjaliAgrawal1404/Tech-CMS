...
if (!isset($_SESSION['email'])) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        echo '<script>alert("Oops!!Details are not found");
    ...
    if (isset($_POST['change'])) {
        // Retrieve and encrypt form inputs
        $current_pwd = md5($_POST['current_password']);
        $new_pwd = md5($_POST['new_password']);
        $confirm_pwd = md5($_POST['confirm_password']);
        $updated_at = date('Y-m-d H:i:s');

        // Validate current password
        if ($current_pwd != md5($row['password'])) {
            echo '<script>
            alert("Your old password is not matched. Please enter correct password.");
